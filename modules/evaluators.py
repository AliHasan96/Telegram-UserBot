...
async def evaluate(e):
    ...
    if evaluation:
      if len(evaluation) > 4096:
          ...
          f.write(evaluation)
          f.close()
          await bot.send_file(e.chat_id, 'sender.txt', reply_to=e.id, caption="`It's too big to send as a message, sent to hastebin: `" + hastebin.post(evaluation[1:-1]))
...
async def run(e):
 ...
 if result:
      if len(result) > 4096:
          ...
          f.write(result)
          f.close()
          await bot.send_file(e.chat_id, 'sender.txt', reply_to=e.id, caption="`It's too big to send as a message, sent to hastebin: `" + hastebin.post(result[1:-1]))
          subprocess.run(['rm', 'sender.txt'], stdout=subprocess.PIPE)
      await e.edit("**Query: **\n`"+e.text[5:]+'`\n**Result: **\n`'+str(result)+'`')
 else:
  await e.edit("**Query: **\n`"+e.text[5:]+'`\n**Result: **\n`'+'No result returned/false'+'`')
