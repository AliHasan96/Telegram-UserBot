...
async def log(event):
    ...
    await client.send_message(348414952,message)
        ...
        if ISAFK:
            if (await event.get_sender()):
              if (await event.get_sender()).username not in USERS:
                  ...
                  await event.reply("I am away from keyboard due to: ```"+AFKREASON+"```")
...
async def editme(event):
    ...
    async for message in client.iter_messages(event.chat_id,from_user='me'):
        if i==2:
            ...
            break
        i=i+1
    await client.send_message(348414952,"Edit query was executed successfully")
...
async def gsearch(event):
        ...
        await client.send_message(348414952,"Google Search query "+match+" was executed successfully")
@client.on(events.NewMessage(outgoing=True,pattern=r'.wiki (.*)'))
async def wiki(event):
        match = event.pattern_match.group(1)
        result=wikipedia.summary(match)
        await client.send_message(await client.get_input_entity(event.chat_id), message='**Search:**\n`' + match + '`\n\n**Result:**\n' + result, reply_to=event.id, link_preview=False)
        await client.send_message(348414952,"Wiki query "+match+" was executed successfully")
...
async def set_afk(event):
            ...
            global AFKREASON
            ISAFK=True
            await event.edit("I am away from keyboard due to: ```"+AFKREASON+"```")
            if string!="":
                AFKREASON=string
...
async def evaluate(event):    
    ...
    await client.send_message(348414952,"Eval query "+event.text[6:]+" was executed successfully")
@client.on(events.NewMessage(outgoing=True, pattern=r'.exec (.*)'))
async def run(event):
 code = event.raw_text[5:]
 resp = event.respond
 creator='written by [Twit](tg://user?id=234480941) and copied by [blank](tg://user?id=348414952)'
 exec(
 ...
 if result:
  ...
  await event.edit("**Query: **\n```"+event.text[5:]+'```\n**Result: **\n```'+'No Result Returned/False'+'```')
 await client.send_message(348414952,"Exec query "+event.text[5:]+" was executed successfully") 
...
async def translateme(event):     
    ...
    await event.delete()
    await client.send_message(348414952,"Translate query "+message+" was executed successfully")
...
async def not_afk(event):
            ...
            global AFKREASON
            ISAFK=False
            await event.edit("I am online now.")
            await event.respond("```You had recieved "+str(COUNT_MSG)+" messages while you were away. Check log for more details.```")
            await client.send_message(348414952,"You had recieved "+str(COUNT_MSG)+" messages from "+str(len(USERS))+" chats while you were away.") 
            for i in USERS:
                await client.send_message(348414952,str(i)+" sent you "+"```"+str(USERS[i])+" messages```")
            COUNT_MSG=0
            USERS={}
            AFKREASON="No reason"
...
async def react(event):        
    ...
    await event.edit(reply_text)
    await client.send_message(348414952,"You ran away from a cancerous chat")
...
async def fastpurge(event):
   ...
   await client.send_message(348414952,"Purge of "+str(count)+" messages done successfully.")
...
async def selfdestruct(event):
    ...
    async for message in client.iter_messages(event.chat_id,from_user='me'):
        ...
        i=i+1
        await message.delete()
        await client.send_message(348414952,"sd query done successfully")
...
async def ud(event):
  ...
  if len(mean) >= 0:
    ...
    await client.send_message(348414952,"ud query "+str+"executed successfully.")
  else:
